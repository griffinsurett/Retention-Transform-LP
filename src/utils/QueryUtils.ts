 ...
 * - fetch ALL items in a collection, filtering out meta files (e.g. files with slugs starting with "_")
 */
export async function getAllItems(collectionName: string) {
  const items = await getCollection(collectionName);
  // Filter out any item whose slug starts with an underscore (like _meta.json)
  return items.filter(item => !item.slug.startsWith('_'));
}
 ...
 *     1) parse route
 *     2) if "getAll", retrieve all items (now filtered)
